...
function getUserLocalHour(timestamp, timezone) {
...
const handler = async () => {
    ...
    for (const segment of SEGMENTS) {
        ...
        let query = db
            .collection("users")
            .where("profile.lastLoginAt", "<", firestore_1.Timestamp.fromMillis(nowDate.getTime() - segment.minDays * 86400000))
            .where("profile.lastLoginAt", ">", firestore_1.Timestamp.fromMillis(nowDate.getTime() - segment.maxDays * 86400000))
            .limit(500);
        ...
        const message = {
            ...
            android: { priority: "high", notification: { channelId: "lumora_retention" } },
            apns: {
        ...
        try {
            ...
            console.log(`Segment ${segment.name}: sent ${response.successCount}/${tokens.length}`);
            // Update counters
